...
public abstract class TFTPPacket {
	...
	public static TFTPPacket getReceivedPacket(ByteBuffer buffer) throws UnsupportedEncodingException{
		//Prepare for reading
		...
		case ACK:
			short blockNumACK = ByteBuffer.wrap(Arrays.copyOfRange(packetBody, 0, 1)).getShort();
			return new ACKPacket(blockNumACK);
		case ERROR:
			short errorCode = ByteBuffer.wrap(Arrays.copyOfRange(packetBody, 0, 1)).getShort();
			return new ERRORPacket(ErrorCode.getErrorCode(errorCode));
		default:
			//Return null for unknown packet type
			return null;
		}
	}
